...
export interface Wallet { id:string; userId:string; balance:number; bonusBalance:number; totalUsed:number; totalReceived:number; reservedCredits:number; planId:string; planStatus?:'free'|'active'|'cancel_at_period_end'|'cancelled'|'past_due'; currentPeriodEnd?:string|null; updatedAt:string; }
